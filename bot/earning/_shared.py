...
def parse_dt(value: Any) -> Optional[datetime]:
    ...
    because remote feeds report ``pubDate`` as an integer, and without it a
    fresh posting reads as having no date at all. Unix timestamps may be
    seconds or milliseconds; 13-digit values are treated as milliseconds.
    """
    if not value:
        return None
    raw = str(value).strip()

    # Epoch seconds or milliseconds. Bounded below so a bare year like "2026"
    # cannot parse as a timestamp in 1970, and above so milliseconds are not
    # read as seconds.
    if raw.isdigit():
        number = int(raw)
        try:
            if len(raw) >= 13 or number > 10**11:
