...
def load_config(section: str, defaults: Mapping[str, Any] | None = None) -> dict:
    ...
    forces tests to monkeypatch globals to change one value.
...
def save_config(section: str, data: dict) -> None:
    """Write one section of the strategy file, merging with existing content.

    Reads the full file, replaces the named section, and writes it back.
    Used by modules that need to persist state changes back to config
    (e.g. updating a refresh timestamp or recording a run result).
    """
    try:
