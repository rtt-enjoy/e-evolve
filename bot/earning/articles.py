...
_DEFAULTS = {
    ...
    "title_min_chars": 25,
    "title_max_chars": 70,
    # Retry configuration for LLM calls that fail with transient errors.
...
_REJECTS = {
    ...
    "llm_rate_limited": "LLM rate-limited after all retries",
...
- Leave one blank line after every heading, before the paragraph that follows.
...
def _call_llm_with_retry(
    ...
    """Call the LLM with exponential-backoff retry for transient failures.

    Rate limits, 503s, and timeout errors are retried up to
    llm_retry_attempts times. Empty responses and 4xx client errors are not
    retried -- they will not resolve on their own.

    Returns the parsed JSON dict, or None when all attempts fail.
    """
    cfg = _config()
    max_attempts = max(1, int(cfg.get("llm_retry_attempts", 3)))
    base_delay = max(1, float(cfg.get("llm_retry_base_delay", 5)))

    last_error = ""
    for attempt in range(1, max_attempts + 1):
        try:
            if hasattr(llm, "complete_json_for_role"):
                data = llm.complete_json_for_role(role, prompt, system=system, max_tokens=max_tokens)
            else:
                data = llm.complete_json(prompt, system=system, max_tokens=max_tokens)

            if data and isinstance(data, dict):
                if attempt > 1:
                    log.info("[articles] LLM succeeded on attempt %d/%d", attempt, max_attempts)
                return data

            # Empty response -- retry only on the first attempt
            if attempt < max_attempts:
                last_error = "empty response"
                delay = base_delay * (2 ** (attempt - 1))
                log.warning("[articles] LLM returned empty response (attempt %d/%d) -- retrying in %.0fs",
            ...
            if is_rate_limit and attempt < max_attempts:
                delay = base_delay * (2 ** (attempt - 1))
                log.warning(
                    "[articles] LLM transient error (attempt %d/%d): %s -- retrying in %.0fs",
                    attempt, max_attempts, last_error[:120], delay,
                )
                time.sleep(delay)
                continue

            # Non-transient error (400 bad request, 401 auth, etc.) -- don't retry
            log.warning("[articles] LLM call failed (attempt %d/%d, not retryable): %s",
                        attempt, max_attempts, last_error[:120])
            if attempt < max_attempts:
                delay = base_delay * (2 ** (attempt - 1))
                time.sleep(delay)
                continue

    log.error("[articles] LLM failed after %d attempts: %s", max_attempts, last_error[:120])
    return None
...
def _generate_article(llm: Any, status: dict) -> Optional[dict]:
    ...
    data = _call_llm_with_retry(llm, prompt, system=_SYSTEM, max_tokens=6000, role="post")
    if not data:
        return _reject("llm_error")
...
def _revise_title(llm: Any, data: dict, problems: list[str]) -> Optional[str]:
    ...
    out = _call_llm_with_retry(llm, prompt, system=_SYSTEM, max_tokens=300, role="post")
    if not out:
        return None
...
def _generate_followup(llm: Any, status: dict, target: dict) -> Optional[dict]:
    ...
    data = _call_llm_with_retry(llm, prompt, system=_FOLLOWUP_SYSTEM, max_tokens=6000, role="post")
    if not data:
        log.warning("[articles] follow-up generation failed: LLM returned nothing after retries")
        return None
...
def _pick_source(status: dict) -> Optional[dict]:
    ...
    # readers turn up for that kind of story. Ordering is only nudged, not
...
def _revise_format(llm: Any, data: dict, problems: list[str]) -> Optional[dict]:
    ...
    out = _call_llm_with_retry(llm, prompt, system=_SYSTEM, max_tokens=6000, role="post")
    if not out or not out.get("body_markdown"):
        return None

    remaining = _format_problems(out["body_markdown"])
    if len(remaining) < len(problems):
        ...
        out.setdefault("tags", data.get("tags", []))
        return out
    return None