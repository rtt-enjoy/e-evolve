...
def _run(status: dict, api_key: str = "", published: list | None = None) -> dict:
    ...
    try:
        ...
        # 'force backfill' bypasses the done_ids track so the owner can
        # re-check articles that were skipped for transient reasons last cycle.
        forced = bool(status.get("_overrides", {}).get("force_backfill"))
        if forced:
            log.info("[backfill] force mode: clearing done_ids, reprocessing all candidates")
            done = set()
        ...
        # Diagnostic log for every candidate -- shows has_footer result and
        # page_views so the owner can see exactly why the queue is (or is not)
        # empty without hunting through the build log.
        for p in posts:
            pid = p.get("id")
            title_short = str(p.get("title", "")[:50])
            views = int(p.get("page_views") or 0)
            in_done = pid in done
            has_fm = devto.has_front_matter(str(p.get("body_markdown") or ""))
            hf = payout.has_footer(str(p.get("body_markdown") or ""), payout_cfg)
            if pid in done:
                log.info(
                    "[backfill] done: id=%s title=%r views=%d -- already footered",
                    pid, title_short, views,
                )
            elif has_fm:
                log.info(
                    "[backfill] skip: id=%s title=%r views=%d -- has YAML front matter (manual edit)",
                    pid, title_short, views,
                )
            elif hf:
                log.info(
                    "[backfill] done: id=%s title=%r views=%d -- already has footer (unknown source)",
                    pid, title_short, views,
                )
            else:
                log.info(
                    "[backfill] pending: id=%s title=%r views=%d -- NO FOOTER, needs append",
                    pid, title_short, views,
                )

        if not candidates:
            # Always set remaining even when there is nothing to do. The previous
            # cycle's count may be stale if the account gained new articles.
            state["remaining"] = 0
            action["success"] = True
            action["_quiet"] = True
            ...
            log.info(
                "[backfill] every published post already carries the footer "
                "(%d total posts checked, %d already done, %d skipped by front-matter or manual edit)",
                len(posts), len(done),
                len(posts) - len(done) - len([p for p in posts if p.get("id") not in done and needs_footer(p, payout_cfg)]),
            )
            return action
        ...
        log.info(
            "[backfill] %d post(s) need footers; updating top %d (of %d max)",
            len(candidates),
            min(len(candidates), int(cfg.get("max_per_cycle", 3))),
            int(cfg.get("max_per_cycle", 3)),
        )
