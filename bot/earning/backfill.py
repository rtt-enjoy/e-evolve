...
def _run(status: dict, api_key: str = "", published: list | None = None) -> dict:
    ...
    try:
        ...
        # Clear resolved 429 entries from skipped whose IDs are now in done_ids.
        # This prevents stale rate-limit errors from polluting the status snapshot
        # after a successful retry in a later cycle.
        skipped = state.get("skipped", {})
        if skipped:
            resolved_ids = set(state["done_ids"])
            for skipped_id in list(skipped.keys()):
                if int(skipped_id) in resolved_ids:
                    del skipped[skipped_id]
                    log.info("[backfill] cleared resolved 429 entry for article %s", skipped_id)
