...
def _run(status: dict, api_key: str = "", published: list | None = None) -> dict:
    ...
    try:
        ...
        base_delay = float(cfg.get("base_retry_delay_seconds", 30))
        
        for post in candidates[:int(cfg.get("max_per_cycle", 5))]:
            body = str(post.get("body_markdown") or "")
            ...
            for attempt in range(max_retries + 1):
                ...
                if "429" in error_msg or "Too Many Requests" in error_msg:
                    if attempt < max_retries:
                        ...
                        log.warning(
                            "[backfill] 429 on post %s, retry %d/%d in %.0fs",
                            post["id"], attempt + 1, max_retries, delay
                        )
                        time.sleep(delay)
                        continue
