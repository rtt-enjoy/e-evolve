...
_PURSUIT_FILE = Path("docs/code-tech-pursuits.md")
...
def run(llm: Any, status: dict[str, Any]) -> list[dict]:
    ...
    if cfg.get("auto_pursue"):
        pursued_count = _auto_pursue(opportunities, cfg)
...
def _auto_pursue(opportunities: list[Opportunity], cfg: dict[str, Any]) -> int:
    """Save actionable Codex prompts for high-scoring channel leads.

    This does NOT post comments, send messages, or take any external action.
    It writes a curated pursuit file (docs/code-tech-pursuits.md) that the owner
    can review and copy-paste into Codex/ChatGPT/Claude to get a working first
    step. This respects the research-only policy while making the top leads
    immediately actionable.
    """
    threshold = int(cfg.get("pursue_score_threshold", 75))
    # Only pursue channel leads (where money changes hands) above threshold
    to_pursue = [
        op for op in opportunities
        if op.kind == _CHANNEL and op.score >= threshold and not op.pursued
    ]
    
    if not to_pursue:
        log.info("[code_techs] no channel leads above pursue threshold %d", threshold)
        return 0

    _PURSUIT_FILE.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Code-Tech Pursuits — Ready for Codex",
        "",
        f"Generated: {datetime.now(timezone.utc).isoformat()}",
        "",
        "These are the highest-scoring CHANNEL leads (places a digital product can be",
        "listed and paid for). Each includes a ready-to-paste Codex prompt that will",
        "produce a concrete first step -- code, config, or checklist -- for that channel.",
        "",
        "**Policy:** This bot does not execute these prompts. The owner copies the prompt",
        "into their AI assistant of choice, reviews the output, and decides whether to",
        "apply it. No external action is taken by the bot.",
        "",
        "---",
        ""
    ]
    ...
    for op in to_pursue:
        ...
        lines.extend([
            f"## {op.title}",
            f"**Score:** {op.score}/100  |  **Source:** {op.source}  |  **URL:** {op.url or 'N/A'}",
            "",
            f"**Why:** {op.reason}",
            f"**Next step:** {op.next_step}",
            f"**Owner must do:** {op.manual_setup or 'nothing'}",
            f"**Cost:** {'free to list' if (op.cost_usd or 0) <= 0 else f'${op.cost_usd:,.2f} one-time'}",
            "",
            "### Codex Prompt",
            "",
            "```",
            op.codex_prompt or _codex_prompt(op, cfg),
            "```",
            "",
            "---",
            ""
    ...
    _PURSUIT_FILE.write_text("\n".join(lines), encoding="utf-8")
    log.info("[code_techs] wrote %d pursuit prompts to %s", len(to_pursue), _PURSUIT_FILE)
    return len(to_pursue)
