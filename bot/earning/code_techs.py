...
# Retry configuration for transient HTTP failures in HN/Reddit fetchers.
_RETRY_MAX_ATTEMPTS = 3
_RETRY_BASE_DELAY = 2  # seconds, exponential backoff base
...
def _request_with_retry(func, max_attempts: int = _RETRY_MAX_ATTEMPTS, base_delay: float = _RETRY_BASE_DELAY):
    """Call ``func()`` with exponential-backoff retry on transient HTTP errors.

    Retries on 429 (rate limit) and 5xx (server errors). Returns the
    response object on success or re-raises the last exception after
    exhausting attempts. This prevents a single temporary network glitch
    from silently dropping an entire source of leads.
    """
    last_exc = None
        ...
        try:
            ...
            if status_code in (429, 500, 502, 503, 504):
                ...
                if attempt < max_attempts - 1:
                    ...
                    log.warning("[code_techs] HTTP %d on attempt %d, retrying in %.1fs", status_code, attempt + 1, delay)
                    time.sleep(delay)
                    continue
                raise
            ...
            if attempt < max_attempts - 1:
                delay = base_delay * (2 ** attempt)
                log.warning("[code_techs] Request error on attempt %d, retrying in %.1fs", attempt + 1, delay)
                time.sleep(delay)
                continue
            raise
    raise last_exc
...
def _fetch_hn_leads(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    """Fetch Hacker News leads via Algolia, with retry on transient failures."""
    leads: list[dict[str, Any]] = []
    ...
    for query in cfg.get("community_searches", []):
        def _hn_request():
            ...
            if resp.status_code in (403, 429):
                raise requests.exceptions.HTTPError(f"HN search skipped ({resp.status_code})", response=resp)
            resp.raise_for_status()
            return resp

        try:
            resp = _request_with_retry(_hn_request)
            data = resp.json()
            if not isinstance(data, dict):
                log.warning("[code_techs] HN returned non-dict response for %r", query)
                continue
            ...
            log.warning("[code_techs] HN search failed for %r after retries: %s", query, exc)
    return leads

def _fetch_reddit_leads(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    """Fetch Reddit leads via RSS, with retry on transient failures."""
    leads: list[dict[str, Any]] = []
    ...
    for subreddit in subreddits:
        for query in queries:
            ...
            def _reddit_request():
                ...
                if resp.status_code in (403, 429):
                    raise requests.exceptions.HTTPError(f"Reddit search skipped ({resp.status_code})", response=resp)
                resp.raise_for_status()
                return resp

            try:
                resp = _request_with_retry(_reddit_request)
                leads.extend(_parse_reddit_rss(resp.text, subreddit))
            except Exception as exc:
                log.warning("[code_techs] Reddit search failed for r/%s %r after retries: %s", subreddit, query, exc)
    return leads
...
def _outreach_draft(title: str, lead: dict[str, Any], value: float, cfg: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"If the result solves the request, the fixed price is "
