...
_LOCAL_LEADS = [
    ...
    {
        "title": "AI-powered resume rewriting service",
        "url": "",
        "source": "local-playbook",
        "body": "Free tiers from Groq, Gemini, and OpenRouter can rewrite resumes to better match job descriptions, highlighting relevant skills and experience. Charge per resume. Buyers are job seekers who currently spend hours tailoring resumes manually.",
        "labels": ["free-ai-api", "resume", "easy-money", "same-day"]
    }
]
...
def _write_report(state: dict[str, Any]) -> None:
    ...
    for index, op in enumerate(state.get("opportunities", []), start=1):
        ...
        lines.extend([
            ...
            f"   - Owner-reviewed outreach draft:",
