...
# Import model validation with graceful fallback
try:
    from .model_fallback import validate_model, fallback_for_model
    _MODEL_FALLBACK_AVAILABLE = True
except Exception:
    _MODEL_FALLBACK_AVAILABLE = False
    def validate_model(role: str, model: str) -> bool:
        return True
    def fallback_for_model(role: str, error: str = "") -> str:
...
def _online_ai_brief(llm: Any, leads: list[dict[str, Any]], cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        if hasattr(llm, "complete_json_for_role"):
            ...
            if _MODEL_FALLBACK_AVAILABLE:
                research_model = "openrouter/free"  # default from status
                if not validate_model("research", research_model):
                    research_model = fallback_for_model("research", "model validation failed")
                    log.info("[code_techs] Using fallback model for research: %s", research_model)
