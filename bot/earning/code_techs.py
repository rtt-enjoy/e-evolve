...
from .product_scaffold import run as scaffold_run
...
_DEFAULT_CONFIG = {
    ...
    # New: automatically generate a product scaffold for the top channel
    "auto_scaffold": True,
    "scaffold_score_threshold": 80,
    "requirements": [
...
def run(llm: Any, status: dict[str, Any]) -> list[dict]:
    ...
    # Auto-scaffold the top channel opportunity if it scores high enough
    if cfg.get("auto_scaffold", True):
        top_channels = [op for op in opportunities if op.kind == _CHANNEL]
        if top_channels:
            best = top_channels[0]
            threshold = int(cfg.get("scaffold_score_threshold", 80))
            if best.score >= threshold:
                log.info("[code_techs] auto-scaffolding top channel: %s (score %d)", best.title, best.score)
                scaffold_result = scaffold_run(status, best.__dict__)
                scaffold_results.extend(scaffold_result)
                if scaffold_result and scaffold_result[0].get("success"):
                    pursued_count += 1
                    # Mark as pursued in the opportunity list
                    for op in opportunities:
                        if op.title == best.title:
                            op.pursued = True
                            break
    ...
    log.info("[code_techs] refreshed %d opportunities, pursued %d, scaffolded %d", len(opportunities), pursued_count, len(scaffold_results))
    results = [{
        "platform": "code_techs",
        "success": True,
        "opportunity_count": len(opportunities),
        "pursued_count": pursued_count,
        "scaffolded_count": len(scaffold_results),
        "estimated_usd": 0.0,
        "target_usd_per_day": state["daily_target_usd"],
        "title": f"Code-tech queue refreshed ({len(opportunities)} leads, {pursued_count} pursued, {len(scaffold_results)} scaffolded)",
