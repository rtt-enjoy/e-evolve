...
_LOCAL_LEADS = [
    ...
    {
        "title": "Semantic search and deduplication service using free Cohere embeddings",
        "url": "",
        "source": "local-playbook",
        "body": "Cohere offers a free tier for embeddings that can be used to build semantic search tools or deduplication services. Charge per dataset or per query for cleaning up customer data, removing duplicate support tickets, or improving internal knowledge bases.",
        "labels": ["free-ai-api", "embeddings", "data-cleanup", "repeatable"]
    }
]
...
def _next_step(text: str) -> str:
    ...
    if any(word in text for word in ("embed", "semantic", "vector")):
        return "Sign up for the free Cohere embeddings tier, process one sample dataset to demonstrate deduplication or search, and publish a fixed price per dataset or per query."
    if _is_free_ai_lead(text):
...
def _write_report(state: dict[str, Any]) -> None:
    ...
    if services:
        lines.extend(["", "## Free AI Services To Use", "",
