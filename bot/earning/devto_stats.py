        ...
        try:
            ...
            if resp.status_code in (429, 500, 502, 503, 504):
                if attempt < max_retries:
                    wait = 2 ** attempt
                    log.info("[devto_stats] retrying after %ds (status %s)", wait, resp.status_code)
                    time.sleep(wait)
                    continue
            ...
            if attempt < max_retries:
                wait = 2 ** attempt
                log.info("[devto_stats] retrying after %ds (error: %s)", wait, exc)
                time.sleep(wait)
            else:
                log.warning("[devto_stats] fetch failed after %d retries: %s", max_retries, exc)
                return []
