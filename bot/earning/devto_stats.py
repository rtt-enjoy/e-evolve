...
_CACHE_FILE = Path(".devto_stats_cache.json")
...
def fetch_published(api_key: str = "") -> list[dict[str, Any]]:
    ...
    if cached is not None:
        log.debug("[devto_stats] returning cached data (%d articles)", len(cached))
        return cached
    ...
    try:
        ...
        if stale is not None:
            log.info("[devto_stats] returning stale cache due to fetch failure")
            return stale
        return []
...
def _read_cache(ignore_ttl: bool = False) -> Optional[list[dict[str, Any]]]:
    """Read cached articles if fresh enough (or if ignore_ttl for stale fallback)."""
...
def _write_cache(articles: list[dict[str, Any]]) -> None:
    """Write articles to cache file with timestamp."""
    try:
        data = {
            "cached_at": time.time(),
            "articles": articles,
        ...
        log.warning("[devto_stats] failed to write cache: %s", exc)
