...
# Cache file for dev.to stats to avoid repeated API calls within a short window
_CACHE_FILE = Path(".devto_stats_cache.json")
_CACHE_TTL_SECONDS = 300  # 5 minutes


def _load_cache() -> dict[str, Any] | None:
    """Load cached dev.to stats if still fresh."""
    ...
    try:
        ...
        if time.time() - cached_at < _CACHE_TTL_SECONDS:
            log.debug("[devto_stats] using cached data (age %.1fs)", time.time() - cached_at)
            return data.get("articles")
    except Exception as exc:
        log.debug("[devto_stats] cache read failed: %s", exc)
    return None


def _save_cache(articles: list[dict[str, Any]]) -> None:
    """Save dev.to stats to cache."""
    try:
        import json
        data = {
            "cached_at": time.time(),
        ...
        log.debug("[devto_stats] cached %d articles", len(articles))
    except Exception as exc:
        log.debug("[devto_stats] cache write failed: %s", exc)
