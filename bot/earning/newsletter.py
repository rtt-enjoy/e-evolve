...
def _ensure_sources(body: str, items: list[dict]) -> str:
    """Make sure every source URL appears in the digest body."""
    for item in items:
...
def _digest_problems(body: str, items: list[dict], cfg: dict) -> list[str]:
    """Return formatting violations specific to a digest."""
    problems: list[str] = []
    min_items = int(cfg.get("min_items", 4))
    if len(items) < min_items:
        problems.append(f"only {len(items)} source(s), need {min_items}+")
    # A digest must have at least one ## section per story.
    section_count = len(re.findall(r"^## ", body, re.MULTILINE))
    if section_count < len(items):
        problems.append(f"missing ## sections: {len(items)} stories, {section_count} sections")
    return problems
...
def _record_issue(status: dict, items: list[dict], limit: int) -> None:
    """Remember this issue's sources so they are never reused."""
    hist = _history(status)
    for item in items:
        bounded_append(hist.setdefault("source_urls", []),
                       trending._canonical_url(item.get("url", "")), limit)
        bounded_append(hist.setdefault("source_titles", []),
                       trending.normalize_title(item.get("title", "")), limit)