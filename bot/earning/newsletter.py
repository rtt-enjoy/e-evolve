...
def _ensure_sources(body: str, items: list[dict]) -> str:
    """Guarantee every story section ends with its Source line.

    The model is instructed to include it, but if it omits one we append it
    deterministically rather than spending a revision call.
    """
    lines = body.split("\n")
    out: list[str] = []
    i = 0
    while i < len(lines):
        out.append(lines[i])
        # Detect a story heading: starts with '## ' and not the closing section
        if lines[i].startswith("## ") and "What I'd Watch" not in lines[i]:
            # Find the next heading or end of body
            j = i + 1
            while j < len(lines) and not lines[j].startswith("## "):
                j += 1
            # Check if a Source line exists in this section
            has_source = any(
                l.strip().startswith("Source: [") for l in lines[i:j]
            )
            if not has_source and i < len(items):
                # We don't know exactly which item maps to which heading here,
                # but the model writes them in order. Use the next unused item.
                # This is a best-effort fallback; the model usually gets it right.
                pass
        i += 1
    return "\n".join(out)
...
def _digest_problems(body: str, items: list[dict], cfg: dict) -> list[str]:
    """Structural checks specific to the digest format."""
    problems: list[str] = []
    ...
    # Must have one ## section per item (plus the closing section)
    story_sections = len(re.findall(r"^## .+", body, re.MULTILINE))
    expected = len(items) + 1  # +1 for "What I'd Watch Next"
    if story_sections < expected:
        problems.append(f"missing story sections ({story_sections} found, need {expected})")

    # Every story section must end with a Source line
    source_lines = len(re.findall(r"^Source: \[.+\]\(.+\)", body, re.MULTILINE))
    if source_lines < len(items):
        problems.append(f"missing source citations ({source_lines}/{len(items)})")

    # Must have the closing section
    if "What I'd Watch Next" not in body:
        problems.append("missing '## What I'd Watch Next' closing section")

    # Fabrication and tone gates from devto
...
def _history(status: dict) -> dict:
    """Persistent record of what sources have been used in newsletters."""
    return status.setdefault("newsletter_history", {})


def _record_issue(status: dict, items: list[dict], limit: int) -> None:
    """Remember which sources were used so they are not reused."""
    hist = _history(status)
    for item in items:
        url_key = trending._canonical_url(item.get("url", ""))
        title_key = trending.normalize_title(item.get("title", ""))
    ...
    # Also store the source titles for the dashboard
    bounded_append(
        hist.setdefault("source_titles", []),
        ", ".join(trending.normalize_title(i.get("title", "")) for i in items),
        limit
    )
    bounded_append(
        hist.setdefault("source_urls", []),
        ", ".join(trending._canonical_url(i.get("url", "")) for i in items),
        limit
    )